...
def train_main(N, alpha, iter, tol, epsilon, in_dim, out_dim, epochs, problem_type, lr,show_plot=False):
    ...
    batches, d0, d1 = dataset_generation(device, N, problem_type=problem_type)
    ...
    print(f"Training for phase 1 beginning")
    with tqdm(total= epochs, desc="Training", unit="epoch", colour='green') as pbar:
        ...
        "NN val ": f"{val:.4e}"
            ...
            #     brea
    print(f"Phase 1 training complete with final loss: {loss.item()}")
    model.epsilon = epsilon
    model._setup_neural_network(hidden_dim=20)
    #model._init_nn()
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    for name, param in model.named_parameters():
        if "nn_modules" not in name:
            param.requires_grad = False
    print(f"Freezing non-NN parameters, only training NN parameters now.")
    print(f"Training for phase 2 beginning")
